...
public class EconomyStorageBackendMySQL extends EconomyStorageBackendCaching {
    ...
    @Override
    public synchronized void reloadUser(String uniqueId) {
        try (Connection conn = dbConn.openConnection()) {
            PreparedStatement ps = dbConn.prepareStatement(conn, String.format("SELECT * FROM `%s` WHERE unique_identifier='%s'", dbConn.getTable("saneeconomy_balances"), uniqueId));
            ResultSet rs = ps.executeQuery();
